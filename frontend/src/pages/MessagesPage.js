...
function MessagesPage() {
    ...
    async (isInitialLoad = false) => {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_BASE}/messages/get_conversations.php`,
          { credentials: "include" }
        );
        if (!response.ok) {
        ...
        const serverConversations = serverConversationsData.map((convo) => ({
          ...
          lastMessageTimestamp: convo.lastMessageTimestamp
            ? new Date(convo.lastMessageTimestamp).toISOString()
            : null,
        }));

        setConversations((currentConversations) => {
          const serverConvoMap = new Map(
            serverConversations.map((convo) => [convo.other_user_id, convo])
          );
          const nextStateConversations = [...serverConversations];

          currentConversations.forEach((currentConvo) => {
            if (
              currentConvo.isPlaceholder &&
              !serverConvoMap.has(currentConvo.other_user_id)
            ) {
          ...
          const uniqueConversationsMap = new Map(
            nextStateConversations.map((convo) => [convo.other_user_id, convo])
          );
        ...
        if (isInitialLoad) {
          setError(
            `Failed to load conversations: ${err.message}. Please refresh.`
          );
          setConversations([]);
        }
        return [];
  ...
  const fetchMessages = useCallback(
    async (otherUserId) => {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_BASE}/messages/get_messages.php?other_user_id=${otherUserId}`,
          { credentials: "include" }
        );
        if (!response.ok) {
          const errorText = await response.text();
  ...
  const handleSelectConversation = useCallback(
    (otherUserId) => {
      ...
      setConversations((prevConversations) => {
        const selectedConvo = prevConversations.find(
  ...
  const handleAddMessage = useCallback(
    async (newMessageData) => {
      ...
      setMessages((prev) => ({
        ...prev,
        [selectedOtherUserId]: [
          ...(prev[selectedOtherUserId] || []),
          optimisticMessage,
        ],
      }));
      setConversations((prev) => {
        const optimisticTimestampISO = optimisticMessage.timestamp;
        const convoExists = prev.some(
          (c) => c.other_user_id === selectedOtherUserId
        );
        let updatedConvos;
        if (convoExists) {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_BASE}/messages/send_message.php`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        if (!response.ok || !result.success)
          throw new Error(
            result.error || `Failed to send message (HTTP ${response.status})`
          );
        ...
        setMessages((prev) => {
          ...
          if (!finalMessages.some((msg) => msg.id === realMessage.id)) {
            const filtered = currentChatMessages.filter(
              (msg) => msg.id !== tempId
            );
              ...
              convo.other_user_id === selectedOtherUserId
                ? {
                    ...
                    lastMessageTimestamp:
                      realMessage.timestamp || new Date().toISOString(),
                    isPlaceholder: false,
                  }
                : convo
            )
        ...
        setMessages((prev) => {
          ...
          return {
            ...
            [selectedOtherUserId]: currentMsgs.map((msg) =>
              msg.id === tempId
                ? { ...msg, isSending: false, error: "Failed to send" }
                : msg
            ),
          };
        });
  ...
  const handleFindAndAddContact = useCallback(
    async (usernameToAdd) => {
      ...
      try {
        const response = await fetch(
          ...
          }/messages/find_user.php?username=${encodeURIComponent(
            usernameToAdd.trim()
          )}`,
          { credentials: "include" }
        );
        ...
        if (!response.ok || !result.success) {
          setAddContactError(
            result.message ||
              result.error ||
              `Could not find user "${usernameToAdd}".`
          );
        } else {
          ...
          setConversations((prev) => {
            ...
            if (alreadyExists) {
              ...
              const newPlaceholderConvo = {
                ...
                userAvatar:
                  foundUser.avatar || "https://i.pravatar.cc/150?img=10",
  ...
  const handleConfirmDeleteChat = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE}/messages/delete_conversation.php`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      setConversations((prev) =>
        prev.filter((c) => c.other_user_id !== userToDelete.id)
      );
      setMessages((prev) => {
        const next = { ...prev };
  ...
  const currentMessages = selectedOtherUserId
  ...
  if (!currentUser && !authLoading) {
    return (
      <div>
        {" "}
        <Header /> <NavBar />{" "}
        <div className="loading-page">Please log in to view messages.</div>{" "}
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="messages-page">
        ...
        <div className="chat-interface-area">
          {error && !isDeleteConfirmModalOpen && (
          ...
          {selectedConversation ? (
            <ChatInterface
              ...
              isLoading={
                isLoadingMessages &&
                selectedOtherUserId === selectedConversation.other_user_id
              }
            ...
            <div className="no-chat-selected">
              ...
              <p>
                Click the '+' button in the Chats list to find someone to
                message, or contact a seller from a listing.
              </p>
            </div>
          ) : !isLoadingConversations ? (
            <div className="no-chat-selected">
